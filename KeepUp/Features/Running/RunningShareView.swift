...
struct RunningShareView: View {
    ...
    var presentation: RunningMapPresentation = .init()
    ...
    @MainActor private func render() async {
        ...
        do {
            let result = try await RunningShareRenderer().render(session: session, locale: locale,
                                                                  satellite: settings.satelliteMap, style: style,
