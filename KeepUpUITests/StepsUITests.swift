...
@MainActor final class StepsUITests: XCTestCase {
    ...
    private func verifySharing(language: String) {
        ...
        share.tap()
        XCTAssertTrue(app.buttons["entry.saveImage"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.descendants(matching: .any).matching(identifier: "steps.share.preview.details").firstMatch.exists)
        XCTAssertTrue(app.buttons["entry.shareSystem"].exists)
        ...
        share.tap()
        XCTAssertTrue(app.buttons["entry.saveImage"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.descendants(matching: .any).matching(identifier: "steps.share.preview.card").firstMatch.exists)
