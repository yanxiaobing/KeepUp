...
已恢复原版 45 种卡片目录和 6 个分类；40 种普通卡可本地打卡，起床和体重专用流程已接通；计步已接入 Core Motion、每日目标、近 7 天读取与达标自动打卡（真机传感器验收待完成）；户外跑步、室内跑步和骑行已接通准备、记录、暂停恢复、保存与历史详情；已补运动设置、倒计时、自动暂停、防误触、常亮及中英文语音，已补分段、速度/步频/海拔指标与曲线、热量、运动附加内容和分页分享，已补三种模式的按月/按日运动汇总，真机验收集中后置。资料填写和会员墙已复用原版资源、布局与交互，并接入本地资料保存。已补充个人信息列表、记录文字/单张照片与草稿、普通卡片详情及分享、14 套主题列表和预览。已补充普通运动卡的热量估算、食物换算和时间线当日热量汇总。已补充自定义卡创建/移除/恢复、固定待打卡、每周进度、本地提醒、未来待办、起床卡和体重目标。其余 UI 正在逐页还原，尚未完成全 App 的 1:1 验收。当前实现和后续范围见[工程基础进度](docs/foundation-status.md)。前期直接使用 WCDB，CloudKit 延后。`Prototypes` 中的代码仅用于技术验证，正式 App 不依赖原型源码。
...
最新进展：[运动统计汇总](docs/running-statistics.md)，此前已完成[运动详情、指标与分享](docs/running-details-sharing.md)，此前已完成[运动设置与语音](docs/running-settings.md)。后续按[功能对齐清单](docs/feature-alignment-plan.md)推进。
